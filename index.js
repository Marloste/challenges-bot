...
const promocommands = [
  {
    ...
    options: [
      ...
      { name: 'add', description: 'Add a member to rotation', type: 1,
        options: [{ name: 'name', description: 'The member name', type: 3, required: true }] },
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (!member.roles.cache.has(HC_ROLE_ID)) return interaction.reply({ content: '❌ This is HC-only.', ephemeral: true });
